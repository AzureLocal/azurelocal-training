...
Slide decks for each training module. Source files (.pptx) are stored in `slides/<module>/` via Git LFS. PDF exports are committed alongside for browser preview.
...
Narration scripts for AI video generation are stored alongside the source PowerPoint files:
```
slides/<module>/narration-script.md
```

Use the `/new-slide-deck <module>` command to scaffold a new deck outline and narration script.
